module "lambda_hjk" {
  ...
  version = "7.7.1"
  function_name = "${local.base_name}-hjk"
  role_name     = "rol-${local.base_name}-hjk"
  handler       = "k"
  runtime       = "java17"
...
module "lambda_hjk_paths" {
  ...
  function_name = module.lambda_hjk.lambda_function_name
  function_arn  = module.lambda_hjk.lambda_function_arn
  priority      = 28
  path_patterns = hjkl
  standard_tags = merge(local.standard_tags, local.lambda_tags)
}
module "lambda_hjk_paths2" {
...
resource "postgresql_role" "lambda_hjk_db_user" {
  name  = module.lambda_hjk.lambda_function_name
  login = true
