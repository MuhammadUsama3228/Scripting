module "lambda_hj" {
  ...
  version = "7.7.1"
  function_name = "${local.base_name}-hj"
  role_name     = "rol-${local.base_name}-hj"
  handler       = "hj"
  runtime       = "java17"
...
module "lambda_hj_paths" {
  ...
  function_name = module.lambda_hj.lambda_function_name
  function_arn  = module.lambda_hj.lambda_function_arn
  priority      = 23
  path_patterns = hj
  standard_tags = merge(local.standard_tags, local.lambda_tags)
}
module "lambda_hj_paths2" {
...
resource "postgresql_role" "lambda_hj_db_user" {
  name  = module.lambda_hj.lambda_function_name
  login = true
