module "lambda_new" {
  ...
  version = "7.7.1"
  function_name = "${local.base_name}-new"
  role_name     = "rol-${local.base_name}-new"
  handler       = "testHandler"
  runtime       = "java17"
...
module "lambda_new_paths" {
  ...
  function_name = module.lambda_new.lambda_function_name
  function_arn  = module.lambda_new.lambda_function_arn
  priority      = 42
  path_patterns = ["/<lambda_path>/*", "/<lambda_path>"]
  standard_tags = merge(local.standard_tags, local.lambda_tags)
}
module "lambda_new_paths2" {
