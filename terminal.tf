module "lambda_terminal" {
  source  = "terraform-aws-modules/lambda/aws"
  version = "7.7.1"
  function_name = "${local.base_name}-terminal"
  role_name     = "rol-${local.base_name}-terminal"
  handler       = "terminal"
  runtime       = "java17"
...
module "lambda_terminal_paths" {
  ...
  function_name = module.lambda_terminal.lambda_function_name
  function_arn  = module.lambda_terminal.lambda_function_arn
  priority      = 12
  path_patterns = terminal
  standard_tags = merge(local.standard_tags, local.lambda_tags)
}
module "lambda_terminal_paths2" {
  count           = var.create_public_endpoints ? 1 : 0
  source          = "../modules/lambda_lb_route"
  maintenance_mode_bypass_code_arn           = var.maintenance_mode_bypass_code_arn
  vpc_id          = var.vpc_id
  lb_listener_arn = aws_alb_listener.api_http.0.arn
  target_name   = "terminal-2"
  function_name = module.lambda_terminal.lambda_function_name
  function_arn  = module.lambda_terminal.lambda_function_arn
  priority      = 12
  path_patterns = terminal
...
resource "postgresql_role" "lambda_terminal_db_user" {
  name  = module.lambda_terminal.lambda_function_name
  login = true
