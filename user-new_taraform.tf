...
module "lambda_user-new_paths" {
  ...
  function_name = module.lambda_user-new.lambda_function_name
  function_arn  = module.lambda_user-new.lambda_function_arn
  priority      = 19
  path_patterns = ["/user", "/user/*"]
  standard_tags = merge(local.standard_tags, local.lambda_tags)
}
module "lambda_user-new_paths2" {
...
resource "postgresql_role" "lambda_user-new_db_user" {
  name  = module.lambda_user-new.lambda_function_name
  login = true
  // RDS iam takes precedence over password auth, so this is disabled immediatly
  password  = "tmp-lambda_user-new_db_user-password"
  superuser = false
