module "lambda_gjh" {
  ...
  version = "7.7.1"
  function_name = "${local.base_name}-hjhl"
  role_name     = "rol-${local.base_name}-hjhl"
  handler       = "kjijihu"
  runtime       = "java17"
...
module "lambda_gjh_paths" {
  ...
  function_name = module.lambda_gjh.lambda_function_name
  function_arn  = module.lambda_gjh.lambda_function_arn
  priority      = 40
  path_patterns = nk
  standard_tags = merge(local.standard_tags, local.lambda_tags)
}
module "lambda_gjh_paths2" {
...
resource "postgresql_role" "lambda_gjh_db_user" {
  name  = module.lambda_gjh.lambda_function_name
  login = true
