module "lambda_usama" {
  ...
  version = "7.7.1"
  function_name = "${local.base_name}-usama"
  role_name     = "rol-${local.base_name}-usama"
  handler       = "usama"
  runtime       = "java17"
...
module "lambda_usama_paths" {
  ...
  function_name = module.lambda_usama.lambda_function_name
  function_arn  = module.lambda_usama.lambda_function_arn
  priority      = 12
  path_patterns = usama
  standard_tags = merge(local.standard_tags, local.lambda_tags)
}
module "lambda_usama_paths2" {
...
resource "postgresql_role" "lambda_usama_db_user" {
  name  = module.lambda_usama.lambda_function_name
  login = true
