module "lambda_your_lambda_name" {
  source  = "terraform-aws-modules/lambda/aws"
  version = "7.7.1"
  function_name = "${local.base_name}-your_lambda_name2"
  role_name     = "rol-${local.base_name}-your_lambda_name2"
...
module "lambda_your_lambda_name_paths" {
  ...
  function_name = module.lambda_your_lambda_name.lambda_function_name
  function_arn  = module.lambda_your_lambda_name.lambda_function_arn
  priority      = 10
  path_patterns = your_lambda_path
  standard_tags = merge(local.standard_tags, local.lambda_tags)
}
module "lambda_your_lambda_name_paths2" {
  count           = var.create_public_endpoints ? 1 : 0
  source          = "../modules/lambda_lb_route"
  maintenance_mode_bypass_code_arn           = var.maintenance_mode_bypass_code_arn
  vpc_id          = var.vpc_id
  lb_listener_arn = aws_alb_listener.api_http.0.arn
  target_name   = "your_lambda_name2-2"
  function_name = module.lambda_your_lambda_name.lambda_function_name
  function_arn  = module.lambda_your_lambda_name.lambda_function_arn
  priority      = 10
  path_patterns = your_lambda_path
...
resource "postgresql_role" "lambda_your_lambda_name_db_user" {
  name  = module.lambda_your_lambda_name.lambda_function_name
  login = true
