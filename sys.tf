module "lambda_sys" {
  ...
  version = "7.7.1"
  function_name = "${local.base_name}-sys"
  role_name     = "rol-${local.base_name}-sys"
  handler       = "sys"
  runtime       = "java17"
...
module "lambda_sys_paths" {
  ...
  function_name = module.lambda_sys.lambda_function_name
  function_arn  = module.lambda_sys.lambda_function_arn
  priority      = 12
  path_patterns = sys
  standard_tags = merge(local.standard_tags, local.lambda_tags)
}
module "lambda_sys_paths2" {
...
resource "postgresql_role" "lambda_sys_db_user" {
  name  = module.lambda_sys.lambda_function_name
  login = true
