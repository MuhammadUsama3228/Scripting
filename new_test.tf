module "lambda_new_test" {
  source  = "terraform-aws-modules/lambda/aws"
  version = "7.7.1"
  function_name = "${local.base_name}-new_test"
  role_name     = "rol-${local.base_name}-new_test"
  handler       = "new_test"
  runtime       = "java17"
...
module "lambda_new_test_paths" {
  ...
  function_name = module.lambda_new_test.lambda_function_name
  function_arn  = module.lambda_new_test.lambda_function_arn
  priority      = 12
  path_patterns = new_test
  standard_tags = merge(local.standard_tags, local.lambda_tags)
}
module "lambda_new_test_paths2" {
  count           = var.create_public_endpoints ? 1 : 0
  source          = "../modules/lambda_lb_route"
  maintenance_mode_bypass_code_arn           = var.maintenance_mode_bypass_code_arn
  vpc_id          = var.vpc_id
  lb_listener_arn = aws_alb_listener.api_http.0.arn
  target_name   = "new_test-2"
  function_name = module.lambda_new_test.lambda_function_name
  function_arn  = module.lambda_new_test.lambda_function_arn
  priority      = 12
  path_patterns = new_test
...
resource "postgresql_role" "lambda_new_test_db_user" {
  name  = module.lambda_new_test.lambda_function_name
  login = true
